...
% posvel, a 6x2 array whose first column is r and second column is v
...
rP = r*cosd(f)*ie + r*sind(f)*ip; % rvec in perifocal coordinates
vP = sqrt(mu/p)*(-sind(f)*ie + (e+cosd(f))*ip);
